...
class GridViewPositionMove(grid_view.GridView):
    def _frame_on_background_latest(self, episode_: agent.Episode):
        last_state: Optional[state.StatePosition] = episode_.last_state
        ...
        last_action: Optional[action.ActionMove] = episode_.last_action
        if last_action:
            agent_move = last_action.move

        prev_state: Optional[state.StatePosition] = episode_.prev_state
        if prev_state:
            prev_position = prev_state.position

        prev_action: Optional[action.ActionMove] = episode_.prev_action
        if prev_action:
    ...
    def _frame_on_background_for_t(self, episode_: agent.Episode, t: int):
        state_: Optional[state.StatePosition] = episode_[t].state
        agent_position: common.XY = state_.position
        agent_move: Optional[common.XY] = None
        prev_position: Optional[common.XY] = None
        prev_move: Optional[common.XY] = None

        last_action: Optional[action.ActionMove] = episode_[t].action
        if last_action:
            agent_move = last_action.move

        if t >= 1 and not self.grid_view_parameters.show_trail:
            prev_state: Optional[state.StatePosition] = episode_[t - 1].state
            prev_position = prev_state.position
            prev_action: Optional[action.ActionMove] = episode_[t - 1].action
            if prev_action:
