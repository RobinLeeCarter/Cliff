...
class GridView(grid_view.GridView):
    def _frame_on_background_latest(self, episode_: agent.Episode):
        ...
        last_action: Optional[action.Action] = episode_.last_action
        if last_action:
        ...
        prev_action: Optional[action.Action] = episode_.prev_action
        if prev_action:
    ...
    def _frame_on_background_for_t(self, episode_: agent.Episode, t: int):
        ...
        last_action: Optional[action.Action] = episode_[t].action
        if last_action:
            agent_move = last_action.move
        ...
        if t >= 1 and not self.grid_view_parameters.show_trail:
            ...
            prev_position = prev_state.position
            prev_action: Optional[action.Action] = episode_[t - 1].action
            if prev_action:
