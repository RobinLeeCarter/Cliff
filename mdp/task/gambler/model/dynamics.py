...
if TYPE_CHECKING:
    ...
    from mdp.task.gambler.model.environment_parameters import EnvironmentParameters
...
from mdp.task.gambler.model.enums import Toss, Result
...
class Dynamics(TabularDynamics[State, Action]):
    ...
    def _build_expected_reward(self):
        ...
        for s, a in self._environment.compatible_s_a:
            state = self._environment.states[s]
            action = self._environment.actions[a]
            self.expected_reward[s, a] = self.get_expected_reward(state, action)
