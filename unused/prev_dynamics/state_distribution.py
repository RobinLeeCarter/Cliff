...
if TYPE_CHECKING:
    from mdp.model.environment.general import general_state
...
class StateDistribution:
    ...
    def __init__(self):
        self._states: list[state.GeneralState] = []          # s'
        ...
        self._state_lookup: dict[state.GeneralState, reward_distribution.RewardDistribution] = {}
    ...
    def add(self, state_: state.GeneralState, reward: float, probability: float):
        """add s', r, p(s',r|s,a)"""
    ...
    def get_state_probability(self, state_: state.GeneralState) -> float:
        """p(s'|s,a)"""
    ...
    def get_state_reward_probability(self, state_: state.GeneralState, reward: float) -> float:
        """returns p(s',r|s,a)"""
        reward_distribution_ = self._state_lookup.get(state_)
        if reward_distribution_:
